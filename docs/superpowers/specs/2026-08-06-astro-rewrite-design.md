...
## Addendum (2026-09-10): cross-browser verification

- Playwright projects now cover Chromium desktop, Pixel 7, WebKit desktop and iPhone 14 WebKit; all 19 tests pass on each engine, including the liquid-glass ready/toggle/theme assertions. Two test fixes came out of it: clipboard permission grants are Chromium-only, and the texture/page consistency check is polled because WebKit settles the page height slightly after the first capture.
- Real Safari (macOS): home and a scrolled blog post screenshotted; liquid glass refracts correctly.
- Firefox: neither Playwright's Firefox build nor Firefox stable can create a profile on this machine ("Could not find profile folder"), so it could not be exercised locally. The Firefox project is opt-in (`PW_FIREFOX=1`) for CI. By feature review the site stays inside Tailwind v4's baseline (Firefox 128+): WebGL, backdrop-filter, `:has()`, `color-mix`, `@property` are all supported, and the scroll-driven `animation-timeline` effects have `@supports` fallbacks that Firefox takes.
