...
export function AwardTimeline({ awards }: AwardTimelineProps) {
  ...
  const getDifficultyConfig = (difficulty: string) => {
    switch (difficulty) {
      case "major":
        return {
          icon: <Medal className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-foreground" />,
          hasSparkle: true
        }
      case "notable":
        return {
          icon: <Star className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-foreground" />,
          hasSparkle: false
        }
      default:
        return {
          icon: <AwardIcon className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-foreground" />,
          hasSparkle: false
        }
    }
  }
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="relative">
        {/* Timeline line */}
        <div className="absolute left-2 sm:left-4 md:left-6 top-6 bottom-6 w-0.5 bg-border" />
              ...
              return (
                ...
                >
                  <div className="absolute left-0 top-2 flex items-center justify-center w-4 h-4 sm:w-8 sm:h-8">
                    <motion.div
                      className="relative w-4 h-4 sm:w-6 sm:h-6 rounded-sm bg-muted border border-border flex items-center justify-center"
        ...
        {/* End marker */}
        <div className="absolute left-1 sm:left-2 md:left-3 bottom-0 w-2 h-2 sm:w-4 sm:h-4 rounded-sm bg-muted border border-border" />
      </div>
    </div>
  );
}
